    ...
    if (osSemaphoreWait(app_event, osWaitForever) == osOK)
    {
      switch (usb_hid_app_state)
      {
        ...
        case USB_HID_APP_START:
          ...
          {
            if (USBH_HID_GetDeviceType(&hUSB_Host) == HID_JOYSTICK || USBH_HID_GetDeviceType(&hUSB_Host) == HID_KEYBOARD)
            {
              USBH_UsrLog("PID: 0x%Xh", hUSB_Host.device.DevDesc.idProduct );
              USBH_UsrLog("VID: 0x%Xh", hUSB_Host.device.DevDesc.idVendor );
